...
- Phase 6-1 scoped source id mismatch를 명시적 repair 옵션으로 attached `.luie` canonical memory entry에 재작성하는 경로는 1차 완료되어 있다.
...
- Phase 7-1 real beta eval runner canonical label option은 1차 완료되어 있으며, `memory:run-eval-suite --real-beta-run-id`로 finalization filter와 같은 label 규칙을 남길 수 있다.
...
- 현재는 **Phase 7-1 threshold candidate calibration, finalization guard, readiness manifest, readiness assertion CLI, real beta label provenance filter, eval runner canonical label option 1차 완료 상태**다.
