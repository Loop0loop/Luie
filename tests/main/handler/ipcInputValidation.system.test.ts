...
import {
  mocked,
  registerAutoSaveInputHandlers,
  registerChapterInputHandlers,
  registerSyncInputHandlers,
  registerWindowInputHandlers,
  resetInputValidationMocks,
} from "./ipcInputValidation.shared";

describe("IPC input validation: window / sync / chapter / autosave", () => {
  beforeEach(() => {
    resetInputValidationMocks();
  });

  it("returns INVALID_INPUT for malformed WINDOW_SET_FULLSCREEN payload", async () => {
    await registerWindowInputHandlers();
  ...
  it("returns INVALID_INPUT for malformed SYNC_SET_AUTO payload", async () => {
    await registerSyncInputHandlers();
  ...
  it("returns INVALID_INPUT for oversized CHAPTER_UPDATE content", async () => {
    ...
    await registerChapterInputHandlers(chapterService);
