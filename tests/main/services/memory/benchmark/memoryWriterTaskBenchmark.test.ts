...
describe("memoryWriterTaskBenchmark", () => {
  ...
  it("requires real beta label provenance when finalization readiness assertion needs it", () => {
    const summary = summarizeMemoryWriterTaskBenchmark([
      {
        evalCase: makeCase("setting", {}),
        scoreResult: makeScore("setting", {}),
    ...
    const manifest = buildMemoryWriterTaskBenchmarkFinalizationManifest({
      ...
      minimumBetaRunCount: 3,
      confirmRealBetaData: true,
      requireRealBetaLabelPrefix: true,
      realBetaLabelPrefix: null,
    });
    ...
    expect(manifest).toMatchObject({
      status: "not_ready",
      realBetaLabelPrefix: null,
      missingRequirements: ["real_beta_label_prefix"],
    });
    expect(
      summarizeMemoryWriterTaskBenchmarkFinalizationReadinessFailures(manifest),
    ).toEqual([
      "Memory writer benchmark finalization is not ready: not_ready",
      "Missing requirement: real_beta_label_prefix",
    ]);
  });

  it("accepts real beta label provenance when finalization readiness assertion needs it", () => {
    const summary = summarizeMemoryWriterTaskBenchmark([
      {
        evalCase: makeCase("setting", {}),
        scoreResult: makeScore("setting", {}),
    ...
    const manifest = buildMemoryWriterTaskBenchmarkFinalizationManifest({
      ...
      confirmRealBetaData: true,
      requireRealBetaLabelPrefix: true,
      realBetaLabelPrefix: MEMORY_WRITER_TASK_REAL_BETA_LABEL_PREFIX,
    });
