...
test("measures write-loop stability on 1000x5000 dataset @stress", async () => {
  ...
  // NOTE: create·seed·burst당 300ms와 queue drain·고정 overhead를 합쳐 timeout을 계산한다.
  ...
  if (!projectId) {
    throw new Error("project.create returned no project id");
  }
  ...
  const profileName = process.env.LUIE_STRESS_PROFILE ?? "custom";
  const outPath = path.join(
    process.cwd(),
    "tests",
    ".tmp",
    `e2e-writing-loop-bench-${profileName}.json`,
  );
  ...
  expect(summary.saveLatencyMs.p95).toBeLessThan(500);
  expect(summary.saveLatencyMs.p99).toBeLessThan(1000);
