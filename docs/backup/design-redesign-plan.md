...
### #2 완료 (2026-06-27) — 크롬 내부 선 제거 + 상단 바 단일화 + 다크 톤 완화
시각 검토 결과 색만 통일하고 선이 남아 "레고 이음새"로 읽힘. + 토글 행과 툴바가 같은 색 이중 띠로 합쳐져 더 나빠 보임 + 순수 검정 에디터가 차갑다는 피드백.
- `MainLayout.tsx`: 사이드바 `border-r` 제거 · 리사이즈 핸들 `bg-border/40`→`bg-transparent`(hover 시만) · **별도 `h-12` 토글 행 삭제**, 두 패널 토글을 에디터 코너에 떠 있는 고스트 버튼으로 통일(캔버스 모드 칩 스타일과 분기). → 툴바가 최상단 단일 크롬 바가 되고 사이드바와 이음새 없이 연결.
- `EditorToolbar.tsx` `border-b` 제거 · `StatusFooter.tsx` `border-t` 제거 → 어두운 에디터 우물은 색 단차로만 정의(이중 경계 제거).
- 다크 토큰 완화: `--bg-app` #09090b→#0d0d0f(에디터 순수 검정 탈피) · `--editor-bg` 동일 정합 · `--bg-sidebar` #121214→#161619(우물 단차 유지, 여전히 panel #18181b 아래) · `--text-primary` #ffffff→#f4f4f5(순백 눈부심 완화).
- 검증: `tsc --noEmit` 통과.
- 남은 후보: 라이트/세피아 동일 검증 · 에디터/캔버스 토글 알약(bg-panel raised) · 우측 컨텍스트 패널 border-l · bg-app 더 따뜻하게/밝게 미세조정(사용자 취향 대기).
