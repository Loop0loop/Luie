...
describe("`.luie` recovery after local DB loss", () => {
  it("reimports project content and snapshots from `.luie` after the sqlite replica is deleted", async () => {
    ...
    expect(restoredChapters[0]?.content).toBeUndefined();
    const restoredChapter = await chapterService.getChapter(restoredChapters[0]!.id);
    expect(restoredChapter.content).toBe(chapterContent);
