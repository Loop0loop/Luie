...
- `src/main/services/features/dbMaintenance/dbMaintenanceService.ts`는 493 LOC입니다.
...
- `src/main/services/features/search/searchService.ts`는 56 LOC입니다.
...
- `settingsManager.ts`의 기존 public export인 `SettingsManager`와 `settingsManager` singleton은 유지했습니다.
- legacy settings path 계산, legacy file migration, legacy window/LLM migration은 `manager/settings/settingsMigration.ts`로 분리했습니다.
...
- `src/renderer/src/features/project/hooks/useFileImport.ts`는 407 LOC입니다.
...
- `useFileImport.ts`의 기존 public export인 `useFileImport` hook signature는 유지했습니다.
- `.luie` world character/term collection schema validation과 import input normalization은 `project/hooks/fileImport/index.ts` 배럴 폴더로 분리했습니다.
...
- `src/main/services/core/chapterService.ts`와 `src/main/services/features/derivedJobs/derivedJobWorker.ts`의 `autoSaveManager` lazy import는 순환 의존 위험이 있어 기존 경로를 유지합니다.
