...
const PATH_KEY_PATTERN =
  /(path|dir|directory|cwd|execPath|userData|datasource|argv)/i;
...
function redactLogData(
  ...
  if (value instanceof Error) {
    ...
    if (cause !== undefined) output.cause = redactLogData(cause, "cause", seen);
...
async function ensureLogFileReady(): Promise<void> {
  ...
  if (!ensureLogFileReadyPromise) {
    ensureLogFileReadyPromise = (async () => {
      ...
      const fs =
        await importNodeModule<NodeFsPromisesModule>("node:fs/promises");
