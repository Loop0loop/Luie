...
export const buildMemoryCanonicalPackagePayload = async (
  ...
  const [entities, aliases, facts, evalCases] = await Promise.all([
    store.select().from(memoryEntity).where(eq(memoryEntity.projectId, projectId)),
    store.select().from(memoryEntityAlias).where(eq(memoryEntityAlias.projectId, projectId)),
    store.select().from(memoryFact).where(eq(memoryFact.projectId, projectId)),
    store.select().from(memoryEvalCase).where(eq(memoryEvalCase.projectId, projectId)),
  ]);
  ...
  const [factEvidence, factInvalidations, evalEvidence, evalEntities, evalRelations] =
    await Promise.all([
      factIds.length === 0
        ? Promise.resolve([])
        : store.select().from(memoryFactEvidence).where(
            and(
      ...
      factIds.length === 0
        ? Promise.resolve([])
        : store.select().from(memoryFactInvalidation).where(
            and(
      ...
      evalCaseIds.length === 0
        ? Promise.resolve([])
        : store.select().from(memoryEvalEvidence).where(
            and(
      ...
      evalCaseIds.length === 0
        ? Promise.resolve([])
        : store.select().from(memoryEvalEntity).where(
            and(
      ...
      evalCaseIds.length === 0
        ? Promise.resolve([])
        : store.select().from(memoryEvalRelation).where(
            and(
  ...
  const episodeEvidence = evidenceIds.length === 0
    ? []
    : await store.select().from(memoryEpisodeEvidence).where(
      and(
  ...
  const episodes = episodeIds.length === 0
    ? []
    : await store.select().from(memoryEpisode).where(
      and(
  ...
  const tables: Partial<Record<MemoryCanonicalTableName, Array<Record<string, unknown>>>> = {
