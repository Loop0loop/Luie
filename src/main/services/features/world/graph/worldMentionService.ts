...
import { getWorldDbClient } from "../entities/characterService.js";
import {
  character,
  faction,
  event,
...
export class WorldMentionService {
    ...
    switch (entityType) {
      case "Character": {
        const results = await client
          .select({ name: character.name, deletedAt: character.deletedAt })
          .from(character)
          .where(eq(character.id, entityId))
          .limit(1);
      ...
      case "Faction": {
        const results = await client
          .select({ name: faction.name, deletedAt: faction.deletedAt })
          .from(faction)
          .where(eq(faction.id, entityId))
          .limit(1);
        const row = results[0] as NamedRow | undefined;
        return row && !row.deletedAt
          ? typeof row.name === "string"
      ...
      case "Event": {
        const results = await client
          .select({ name: event.name, deletedAt: event.deletedAt })
          .from(event)
      ...
      case "Term": {
        const results = await client
          .select({ term: term.term, deletedAt: term.deletedAt })
          .from(term)
          .where(eq(term.id, entityId))
          .limit(1);
      ...
      case "WorldEntity": {
        const results = await client
          .select({ name: worldEntity.name })
          .from(worldEntity)
          .where(eq(worldEntity.id, entityId))
          .limit(1);
        const row = results[0] as NamedRow | undefined;
    ...
    const chapters = await client
      ...
      .where(
        and(
          ...
          eq(chapter.projectId, query.projectId),
  ...
  private async getFallbackMentions(
    ...
    const chapters = await client
      ...
      .from(chapter)
      .where(
        and(eq(chapter.projectId, query.projectId), isNull(chapter.deletedAt)),
      )
