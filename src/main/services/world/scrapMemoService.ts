...
class ScrapMemoService {
  async createScrapMemo(input: ScrapMemoCreateInput) {
    try {
      ...
      const [created] = await db.getClient().insert(scrapMemo).values({
      ...
      await projectService.persistPackageAfterMutation(input.projectId, "scrap-memo:create");
  ...
  async updateScrapMemo(input: ScrapMemoUpdateInput) {
    try {
      ...
      await projectService.persistPackageAfterMutation(String(updated.projectId), "scrap-memo:update");
      return {
        ...updated,
  ...
  async deleteScrapMemo(id: string) {
    try {
      ...
      await projectService.persistPackageAfterMutation(String(current.projectId), "scrap-memo:delete");
      return { success: true };
    } catch (error) {
