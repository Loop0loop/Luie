  ...
  return (
    <div className="flex flex-col h-screen w-screen bg-app text-fg overflow-hidden relative">
      ...
      <div className="flex-1 overflow-hidden relative">
        ...
        {/* RIGHT: 바인더바 (hover 시 표시, 리사이즈 가능) */}
        <FocusHoverSidebar side="right" topOffset={sidebarTopOffset} isResizing={isResizing}>
          <div className="h-full flex flex-row bg-panel border-l border-border">
            {/* 패널 콘텐츠 영역 */}
            <div
              className="flex flex-col overflow-hidden bg-panel shrink-0 relative"
              style={{
                width: activeRightTab ? `${currentWidth}px` : "0px",
                transition: isResizing ? "none" : activeRightTab ? "none" : "width 0.3s",
              }}
            >
              {activeRightTab && (
        ...
        <div className="h-full w-full overflow-y-auto bg-[#f3f4f6] dark:bg-[#1a1a1a] flex flex-col items-center custom-scrollbar">
          {/* A4 페이지 - overflow-hidden 제거해서 스크롤 가능하게 */}
          <div className="w-[816px] min-h-[1056px] bg-white dark:bg-[#1e1e1e] shadow-2xl border border-black/5 dark:border-white/5 py-12 px-12 my-8 transition-all duration-200 ease-out">
            ...
            {/* 에디터 콘텐츠 */}
            <div className="min-h-[500px] [&_.ProseMirror]:outline-none [&_.ProseMirror]:min-h-[400px]">
              {children}
            </div>
          </div>
