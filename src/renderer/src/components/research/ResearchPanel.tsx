...
function MemoSection() {
  const [notes, setNotes] = useState([
    { id: '1', title: '참고자료: 중세 복식', content: '링크: https://wiki...\n\n중세 귀족들의 의상은 생각보다 화려했다...', tags: ['자료', '의상'], updatedAt: new Date().toISOString() },
    { id: '2', title: '아이디어 파편', content: '- 주인공이 사실은 악역이었다면?\n- 회귀 전의 기억이 왜곡된 것이라면?', tags: ['아이디어', '플롯'], updatedAt: new Date().toISOString() }
  ]);
    ...
    <div className={styles.scrapContainer}>
      <div className={styles.noteList}>
        ...
        <div style={{padding: '8px 12px', borderBottom: '1px solid var(--border-default)'}}>
          <div style={{display:'flex', alignItems:'center', gap: 8, backgroundColor: 'var(--bg-element)', padding: '6px 8px', borderRadius: 4}}>
            <Search size={12} color="var(--text-tertiary)"/>
            <input 
              style={{border:'none', background:'transparent', outline:'none', fontSize: 12, width: '100%', color:'var(--text-primary)'}}
              placeholder="Search..."
              value={searchTerm}
        ...
        {filteredNotes.map(note => (
          ...
          >
            <div style={{fontWeight: 500, marginBottom: 4}}>{note.title || 'Untitled'}</div>
            <div style={{display:'flex', gap: 4, flexWrap:'wrap', marginBottom: 4}}>
              {note.tags.map(t => (
                <span key={t} style={{fontSize: 9, padding: '2px 4px', background: 'var(--bg-element-hover)', borderRadius: 2}}>{t}</span>
              ))}
            </div>
            <div style={{
      ...
      {activeNote ? (
        <div className={styles.noteContent}>
          ...
          <div style={{padding: '12px 24px 0 24px', display:'flex', gap: 8, alignItems:'center'}}>
            ...
            <input 
              style={{border:'none', background:'transparent', outline:'none', fontSize: 12, color:'var(--text-secondary)', width:'100%'}}
          ...
          <input 
            ...
            onChange={(e) => setNotes(notes.map(n => n.id === activeNoteId ? {...n, title: e.target.value, updatedAt: new Date().toISOString()} : n))} 
            placeholder="Title"
          />
          <textarea 
            className={styles.noteBodyInput}
            value={activeNote.content}
            onChange={(e) => setNotes(notes.map(n => n.id === activeNoteId ? {...n, content: e.target.value, updatedAt: new Date().toISOString()} : n))} 
