      ...
      <div className="w-[960px] h-[640px] bg-bg-element rounded-xl shadow-lg border border-black/5 flex overflow-hidden max-h-[95vh] relative animate-in slide-in-from-bottom-5 duration-200" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex w-full h-full">
            ...
            <div className="flex-1 px-3 flex flex-col gap-1 overflow-y-auto">
              {sidebarItems.map((item) => (
                <div
                  key={item.id}
                  className={cn(
                    "px-4 py-3 text-sm text-muted rounded-md cursor-pointer transition-all font-medium hover:bg-active hover:text-fg",
                    activeTab === item.id && "bg-active text-fg font-semibold"
                  )}
          ...
          <div className="flex-1 flex flex-col bg-bg-element relative overflow-hidden">
            ...
            {activeTab === "editor" && (
              <div className="flex-1 px-[60px] py-12 overflow-y-auto flex flex-col gap-8">
                {/* FONT FAMILY */}
                <div className="flex flex-col gap-3">
                  <div className="text-[13px] font-semibold text-muted uppercase tracking-wide mb-1">{SETTINGS_SECTION_FONT}</div>
                ...
                {/* OPTIONAL FONTS */}
                <div className="flex flex-col gap-3">
                  <div className="text-[13px] font-semibold text-muted uppercase tracking-wide mb-1">{SETTINGS_SECTION_OPTIONAL_FONTS}</div>
                        ...
                        <div key={font.id} className="flex items-center justify-between p-2.5 px-3 border border-border rounded-lg bg-bg-element">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col gap-0.5">
                              ...
                              <div className="text-[11px] text-tertiary">{font.family}</div>
                            </div>
                          </div>
                ...
                <div className="flex flex-col gap-3">
                  <div className="flex flex-col gap-3">
                    <div className="flex justify-between items-center">
                      <div className="text-[13px] font-semibold text-muted uppercase tracking-wide mb-1">{SETTINGS_SECTION_FONT_SIZE}</div>
                      <div className="text-sm font-semibold text-active bg-active/10 px-2 py-0.5 rounded">{localFontSize}px</div>
                    </div>
                    <input
                      type="range"
                      min="14"
                      ...
                      step="1"
                      value={localFontSize}
                      className="w-full h-1 bg-border rounded-sm appearance-none cursor-pointer outline-none [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-5 [&::-webkit-slider-thumb]:h-5 [&::-webkit-slider-thumb]:bg-white [&::-webkit-slider-thumb]:border-2 [&::-webkit-slider-thumb]:border-active [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:shadow-sm [&::-webkit-slider-thumb]:transition-transform hover:[&::-webkit-slider-thumb]:scale-110"
                  ...
                  <div className="flex flex-col gap-3">
                    <div className="flex justify-between items-center">
                      <div className="text-[13px] font-semibold text-muted uppercase tracking-wide mb-1">{SETTINGS_SECTION_LINE_HEIGHT}</div>
                      <div className="text-sm font-semibold text-active bg-active/10 px-2 py-0.5 rounded">{localLineHeight}</div>
                    </div>
                    <input
                      type="range"
                      min="1.4"
            ...
            {activeTab === "appearance" && (
              <div className="flex-1 px-[60px] py-12 overflow-y-auto flex flex-col gap-8">
                <div className="flex flex-col gap-3">
                  <div className="text-[13px] font-semibold text-muted uppercase tracking-wide mb-1">{SETTINGS_SECTION_THEME}</div>
                  <div className="grid grid-cols-3 gap-3">
                    {(["light", "sepia", "dark"] as EditorTheme[]).map((t) => (
                      <button
                        key={t}
                        className={cn(
                          "h-20 rounded-xl border-2 border-transparent cursor-pointer relative flex items-center justify-center text-sm font-semibold transition-all hover:scale-[1.02]",
                          t === "light" && "bg-white border-[#e0e0e0] text-[#333]",
                          t === "sepia" && "bg-[#fbf0d9] border-[#f0e6d2] text-[#5f4b32]",
                          t === "dark" && "bg-[#222] border-[#333] text-[#eee]",
                          theme === t && "border-active"
                        )}
                        onClick={() => updateSettings({ theme: t })}
                      >
                        {theme === t && (
                          <div className="absolute top-1.5 right-1.5 bg-active text-white w-[18px] h-[18px] rounded-full flex items-center justify-center">
